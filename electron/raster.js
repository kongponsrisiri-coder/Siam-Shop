...
function packBitmap({ width, height, data, channels = 4, order = 'rgba', threshold = 140 }) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (a < 128) continue; // transparent → paper (white)
      const lum = data[o + rIdx] * 0.3 + data[o + gIdx] * 0.59 + data[o + bIdx] * 0.11;
      if (lum < threshold) bytes[y * widthBytes + (x >> 3)] |= 0x80 >> (x & 7);
    }
  }
...
module.exports = { packBitmap, gsv0, bitmapToEscPos, hashString, MAX_WIDTH_DOTS };
